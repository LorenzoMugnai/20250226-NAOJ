...
class Inspector:
    ...
    def prune_incomplete_planets(self):
        """Removes from the table all the planets that do not have retrieval results for all the models"""
        
        # Get unique planet base names (removing suffixes like Mig, Fidu, etc.)
        suffixes = ["Mig", "Fidu", "MigR", "FiduR"]
        base_names = []
        
        for name in self.out_targetlist["Planet Name"]:
            base_name = name
        ...
        # Count combinations for each base planet name
        unique_base_names = list(set(base_names))
        planets_to_keep = []
        
        for base_name in tqdm(unique_base_names, total=len(unique_base_names)):
            # Get indices for this planet
            indices = [i for i, bn in enumerate(base_names) if bn == base_name]
            
            # Check if we have all 4 combinations
            combinations_found = set()
            
            for idx in indices:
                migration = self.out_targetlist["Migration Efficiency [Fiducial = 0, Efficient = 1]"][idx]
                redox = self.out_targetlist["Redox [Reduced = 0, Oxidized = 1]"][idx]
                combinations_found.add((migration, redox))
            
            # If we have all 4 combinations (0,0), (0,1), (1,0), (1,1), keep this planet
            expected_combinations = {(0, 0), (0, 1), (1, 0), (1, 1)}
            
            if combinations_found == expected_combinations:
                planets_to_keep.extend(indices)
            else:
                print(f"Removing incomplete planet {base_name}: found {len(combinations_found)}/4 combinations: {combinations_found}")
        
        # Keep only complete planets
        planets_to_keep.sort()
        self.out_targetlist = self.out_targetlist[planets_to_keep]
        
        print(f"Kept {len(set([base_names[i] for i in planets_to_keep]))} complete planets with {len(planets_to_keep)} total rows")
    ...
    def plot_distance_panels(tbl, param_base=["O/H", "C/O", "He/H"], combo="00_vs_11", ylim=10, title=None):
        ...
        color_map = {'M': '#e41a1c', 'G': '#377eb8', 'K': '#ff7f00'}
